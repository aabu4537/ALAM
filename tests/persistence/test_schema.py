...
def test_expected_tables_exist_and_nothing_else(session: Session) -> None:
    ...
    — see ADR-0009). `content_chunks` remains later work — a stray table here
    means something was built ahead of the chunking pipeline that doesn't
    exist yet (see ADR-0008)."""
    tables = set(
        session.scalars(
            text(
