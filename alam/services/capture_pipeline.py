"""Job handlers for the M2 capture pipeline: transcribe, then correct against
the book's entity list. Extraction (M2 session 3) is the next stage after
correction, chained the same way. Each stage is its own job so a failure in
one does not force the others to redo — retried independently, same as any
job in the queue (CLAUDE.md rule 5).
